...
1. Going around the track
    - Generate five points in Frenet coordinates we want the path to pass through.
        - First two points: last two points in the previous path
...
3. Driving with velocity under (but close to) the speed limit
    - Set a `ref_velocity` slightly lower than the speed limit. This is the speed we want our car to drive at. Max speed limit set to 49 MPH.
...
6. Changing lanes
    ...
    - Change lanes.
        - Implementation: Change our car's `lane` variable from the current lane to the target lane.
